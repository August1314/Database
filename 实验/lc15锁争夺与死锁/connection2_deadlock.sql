...
-- 2. 同时（或几乎同时）在连接1中执行 connection1_deadlock.sql
...
PRINT '===============================================================';
PRINT '实验说明：';
PRINT '1. 如果连接2成功，说明连接1被选作死锁牺牲品';
PRINT '2. 如果连接2失败（错误1205），说明连接2被选作死锁牺牲品';
PRINT '3. 死锁牺牲品的事务会被自动回滚';
PRINT '4. 成功的事务会正常提交并更新数据';
PRINT '';
PRINT '死锁原因：';
PRINT '- 两个连接都通过SELECT获取共享锁';
PRINT '- 两个连接都尝试通过UPDATE将共享锁升级为排他锁';
PRINT '- 由于REPEATABLE READ隔离级别，共享锁在事务完成前不能释放';
PRINT '- 形成循环等待，导致死锁';
PRINT '===============================================================';
